...
<head>
    <meta charset="UTF-8">
    <title>Вхід в акаунт - Yacht Club</title>
...
<body class="flex items-center justify-center min-h-screen p-6">
    ...
    <div class="w-full max-w-md bg-[#1a6668] p-8 rounded-3xl shadow-2xl border border-[#2b8a8c] text-white">
        ...
        @if(session('success'))
            <div class="mb-4 p-4 bg-green-600/30 border border-green-400 text-green-200 rounded-xl text-center">
                {{ session('success') }}
            </div>
        @endif
        ...
        <form action="{{ route('client.login') }}" method="POST">
            @csrf
            <h3 class="text-xl font-bold mb-4 text-cyan-100">Вхід в акаунт</h3>
            ...
            <button type="submit" class="w-full py-3 bg-[#0a3536] hover:bg-[#0f3d3e] text-white font-bold rounded-full btn-border transition">
                Увійти
            </button>
        </form>
