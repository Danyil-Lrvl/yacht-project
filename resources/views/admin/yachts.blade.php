    ...
    <form action="{{ route('admin.yacht.store') }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="p-4 bg-[#0f3d3e] rounded-2xl border border-[#2b8a8c]">
            ...
            <div class="mb-4">
                ...
                <input type="text" name="name" value="{{ old('name') }}" placeholder='напр:Nordhavn 42 - "Марія"' required class="w-full p-3 rounded-xl bg-[#1a6668] border border-[#2b8a8c] text-white">
                @error('name')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                <div>
                    ...
                    <select name="type_id" required class="w-full p-3 rounded-xl bg-[#1a6668] border border-[#2b8a8c] text-white">
                        @foreach($types ?? [] as $t)
                            <option value="{{ $t->id_type ?? $t->id }}" {{ old('type_id') == ($t->id_type ?? $t->id) ? 'selected' : '' }}>{{ $t->name_type }}</option>
                        @endforeach
                    </select>
                    @error('type_id')
                        <span class="text-rose-400 text-xs mt-1 block">{{ $message }}</span>
                ...
                <div>
                    ...
                    <input type="text" name="serial_number" value="{{ old('serial_number') }}" placeholder="NH42-00000" required class="w-full p-3 rounded-xl bg-[#1a6668] border border-[#2b8a8c] text-white">
                    @error('serial_number')
                        <span class="text-rose-400 text-xs mt-1 block">{{ $message }}</span>
                    @enderror
                </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-4">
                <div>
                    <label class="block mb-2 text-xs">Рік випуску:</label>
                    <input type="number" name="year" value="{{ old('year', 2026) }}" class="w-full p-3 rounded-xl bg-[#1a6668] border border-[#2b8a8c] text-white">
                    @error('year')
                        <span class="text-rose-400 text-xs mt-1 block">{{ $message }}</span>
                    @enderror
                </div>
                <div>
                    ...
                    <input type="number" step="0.01" name="price_rent" value="{{ old('price_rent') }}" required class="w-full p-3 rounded-xl bg-[#1a6668] border border-[#2b8a8c] text-white">
                    @error('price_rent')
                        <span class="text-rose-400 text-xs mt-1 block">{{ $message }}</span>
                    @enderror
                </div>
                <div>
                    ...
                    <input type="number" step="0.01" name="price_buy" value="{{ old('price_buy') }}" required class="w-full p-3 rounded-xl bg-[#1a6668] border border-[#2b8a8c] text-white">
                    @error('price_buy')
                        <span class="text-rose-400 text-xs mt-1 block">{{ $message }}</span>
                    @enderror
                </div>
                <div>
                    ...
                    <select name="type_oper" class="w-full p-3 rounded-xl bg-[#1a6668] border border-[#2b8a8c] text-white">
                        <option value="rent" {{ old('type_oper') == 'rent' ? 'selected' : '' }}>rent (оренда)</option>
                        <option value="buy" {{ old('type_oper') == 'buy' ? 'selected' : '' }}>buy (купівля)</option>
                    </select>
                    @error('type_oper')
                        <span class="text-rose-400 text-xs mt-1 block">{{ $message }}</span>
                    @enderror
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                <div>
                    <label class="block mb-2 text-sm">Останнє обслуговування:</label>
                    <input type="text" name="last_maintenance" value="{{ old('last_maintenance', date('Y-m-d')) }}" class="w-full p-3 rounded-xl bg-[#1a6668] border border-[#2b8a8c] text-white">
                </div>
                <div>
                    <label class="block mb-2 text-sm">Дата реєстрації:</label>
                    <input type="text" name="registration_date" value="{{ old('registration_date', date('Y-m-d')) }}" class="w-full p-3 rounded-xl bg-[#1a6668] border border-[#2b8a8c] text-white">
                </div>
                <div>
                    ...
                    <select name="is_active" class="w-full p-3 rounded-xl bg-[#1a6668] border border-[#2b8a8c] text-white">
                        <option value="1" {{ old('is_active') == '1' ? 'selected' : '' }}>Активна (1)</option>
                        <option value="0" {{ old('is_active') == '0' ? 'selected' : '' }}>Неактивна (0)</option>
                    </select>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                <div>
                    ...
                    <input type="text" name="comment" value="{{ old('comment', 'Стандартна комплектація') }}" class="w-full p-3 rounded-xl bg-[#1a6668] border border-[#2b8a8c] text-white">
                </div>
                <div>
                    <label class="block mb-2 text-sm">Фотографія яхти:</label>
                    <input type="file" name="image" class="w-full p-2.5 rounded-xl bg-[#1a6668] border border-[#2b8a8c] text-white file:mr-4 file:py-2 file:px-4 file:rounded-xl file:border-0 file:text-sm file:font-semibold file:bg-cyan-400 file:text-[#0f3d3e] hover:file:bg-cyan-300">
                    @error('image')
                        <span class="text-rose-400 text-xs mt-1 block">{{ $message }}</span>
                    @enderror
                </div>
...
<div class="bg-[#1a6668] p-8 rounded-3xl border border-[#2b8a8c]">
    ...
    <form action="{{ route('admin.yacht.update', $yacht->id) }}" method="POST" enctype="multipart/form-data" class="mb-6 p-6 bg-[#0f3d3e] rounded-2xl border border-[#2b8a8c]">
        @csrf
        @method('PUT')
        ...
        <div class="flex items-center justify-between mb-4 pb-3 border-b border-[#2b8a8c]">
            ...
            @if(!empty($yacht->image))
                <div>
                    <img src="{{ asset('images/' . $yacht->image) }}" alt="Фото" class="w-16 h-16 object-cover rounded-xl border border-[#2b8a8c]">
                </div>
            @endif
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
            ...
            <div>
                <label class="block mb-2 text-xs">Змінити фото:</label>
                <input type="file" name="image" class="w-full p-2 rounded-xl bg-[#1a6668] border border-[#2b8a8c] text-white file:mr-2 file:py-1 file:px-3 file:rounded-lg file:border-0 file:text-xs file:font-semibold file:bg-cyan-400 file:text-[#0f3d3e] hover:file:bg-cyan-300">
            </div>
        </div>
