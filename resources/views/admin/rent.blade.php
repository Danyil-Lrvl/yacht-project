...
@section('title', 'Керування орендою | Адмін-панель')

@section('content')
<div class="bg-[#1a6668] p-8 rounded-3xl border border-[#2b8a8c] mb-10">
    <h2 class="text-2xl font-bold mb-6 text-cyan-300">Керування замовленнями (Оренда)</h2>
    
    <!-- Фільтри -->
    <form method="GET" action="{{ route('admin.rent') }}" class="flex gap-4 mb-6 items-center flex-wrap">
        <div>
        ...
        <div class="self-end flex gap-2">
            ...
            <a href="{{ route('admin.rent') }}" class="px-4 py-2 bg-[#1d5f64] text-white rounded-lg">Скинути</a>
        </div>
    </form>
        ...
        <table class="w-full text-left border-collapse">
            ...
            <tbody>
                ...
                <tr class="border-b border-[#2b8a8c]/50">
                    <td class="p-3 text-cyan-200">{{ $rent->created_date ?? $rent->created_at->toDateString() }}</td>
                    <td class="p-3">
                        <details class="cursor-pointer">
                            <summary class="font-semibold text-white hover:text-cyan-300">{{ $rent->client->full_name ?? 'Клієнт не знайдений' }}</summary>
                            <div class="mt-2 p-2 bg-[#0f3d3e] rounded-lg text-xs space-y-1 border border-[#2b8a8c]">
                                <p><strong>ПІБ:</strong> {{ $rent->client->full_name ?? '—' }}</p>
                                <p><strong>Телефон:</strong> {{ $rent->client->phone ?? '—' }}</p>
